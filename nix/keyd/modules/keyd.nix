  ...
  config = mkIf cfg.enable {
    environment = {
      etc = lib.attrsets.mapAttrs' (
        name: tcfg: nameValuePair "keyd/${name}.conf" { text = tcfg; }
      ) cfg.configuration;
    };
    ...
    systemd.services.keyd = {
      ...
      restartTriggers = builtins.map (name: config.environment.etc."keyd/${name}.conf".source) (
        builtins.attrNames cfg.configuration
      );
